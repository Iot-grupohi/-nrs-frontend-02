...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Verifica se a URL tem o parâmetro auto=true (acesso direto via raiz)
        const urlParams = new URLSearchParams(window.location.search);
        const isAutoAccess = urlParams.get('auto') === 'true';
        
        // Configura listener de autenticação
        auth.onAuthStateChanged(user => {
            // Se veio via acesso direto (auto=true), não redireciona automaticamente
            // Isso permite que o usuário veja a tela de login mesmo estando autenticado
            if (user && user.emailVerified && !isAutoAccess) {
                // Usuário já está logado e verificado, redirecionar para o dashboard
                window.location.href = '/index.html';
            } else if (user && user.emailVerified && isAutoAccess) {
                // Se o usuário está autenticado mas acessou via raiz, mostra aviso
                showSuccess('Você já está autenticado. Clique no botão abaixo para acessar o dashboard ou faça logout para usar outra conta.');
                
                // Adiciona botão para ir ao dashboard
                const loginPanel = document.getElementById('login-panel');
                if (loginPanel && !document.getElementById('goto-dashboard-btn')) {
                    const dashboardBtn = document.createElement('div');
                    dashboardBtn.id = 'goto-dashboard-btn';
                    dashboardBtn.className = 'd-grid mt-3';
                    dashboardBtn.innerHTML = `
                        <a href="/index.html" class="btn btn-success">
                            <i class="fas fa-home me-2"></i>Ir para o Dashboard
                        </a>
                        <button type="button" id="force-logout-btn" class="btn btn-outline-secondary mt-2">
                            <i class="fas fa-sign-out-alt me-2"></i>Sair e usar outra conta
                        </button>
                    `;
                    loginPanel.appendChild(dashboardBtn);
                    
                    // Adiciona evento para logout forçado
                    document.getElementById('force-logout-btn').addEventListener('click', async () => {
                        try {
                            await auth.signOut();
                            showSuccess('Logout realizado. Faça login com outra conta.');
                            dashboardBtn.remove();
                        } catch (error) {
